...
hist_csv_file = 'FCN8_white.csv'
with open(hist_csv_file, mode='w') as f:
    hist_df.to_csv(f)
    
model.save_weights("camvid_model_FCN8_white.h5", overwrite=True)
...
plt.ylabel("Accuracy")
plt.legend(loc="lower left")
plt.savefig("FCN8_result_static_e"+str(num_epochs))
plt.show()
...
for i in range(0,np.shape(pred_all)[0]):
    ...
    #ax3.grid(b=None)
    plt.savefig("FCN8_result_"+str(i))
    plt.show()
